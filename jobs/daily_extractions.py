...
loggerFileName = f'yahoo_stats_{datetime.date.today().strftime("%b-%d-%Y")}.log'
...
for each_item in items:
    if each_item.endswith("{}.sql".format(runtime)):
        if upload_to_bucket(each_item, os.path.join(jcfg.JOB_ROOT, "sql_outputs", each_item),
                            'bobanalytics.appspot.com'):
