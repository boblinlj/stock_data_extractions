...
class CalculateRank:
    ...
    def combine_strategy(self):
        ...
        for strategy in self.model_json.get('strategies'):
            ...
            st_weighteds.append(st_weighted)

            if len(st_filter) > 0:
                self.factor_ranking_pct[st_weighted] = self.factor_ranking_pct.loc[
                                                           lambda df: eval(strategy.get("strategy_filter"))][st_rank] * st_weight
            else:
                self.factor_ranking_pct[st_weighted] = self.factor_ranking_pct[st_rank] * st_weight

        self.factor_ranking_pct['ms_average'] = self.factor_ranking_pct[st_weighteds].sum(axis=1)
        bs = np.array(bucket_threshold)
        self.factor_ranking_pct['bucket_count'] = (np.sign(self.factor_ranking_pct[st_weighteds] - bs) + 1).sum(axis=1) / 2
        ...
        self.final_model_output = self.factor_ranking_pct[strategy_names + st_weighteds + ['final_score', 'ms_pct', 'bucket_count']]
...
class RunModel:
    def __init__(self, process_dt, updated_dt, model_name, loggerFileName=None):
        ...
        self.targeted_table = 'model_1_weekly_results'
    ...
    def run_model(self):
        ...
        model_out.to_csv('test.csv')

        DatabaseManagement(data_df=model_out, table='model_1_weekly_results', insert_index=True).insert_db()
...
if __name__ == '__main__':
    obj = RunModel(process_dt='2022-02-18', updated_dt='2022-02-20', model_name='model_config.json')
    obj.run_model()
